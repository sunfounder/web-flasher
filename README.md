...
SunFounder 固件烧录工具集：一个页面收纳所有需要浏览器烧录的工具/固件，**Chrome / Edge 打开即用**
（Web Serial 直连设备，无需安装驱动与软件），支持 ESP32 / S2 / S3 / C3 等系列。
当前工具会随 `firmwares.json` 逐步扩充。

在线地址：https://sunfounder.github.io/web-flasher/

## 使用
1. 用 Chrome 或 Edge 打开本页（必须 https 或 localhost）。
2. 下拉选择要烧录的工具/固件 → 「连接并刷机」→ 弹窗里选设备串口。
